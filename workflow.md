...
| 2. UpNote | `🔄 UpNote 저장 (주)` | 이번 주 전체를 1개 노트로 묶어 UpNote 앱 생성 (옵션) |
| 3. Notion | `🔄 Notion 동기화 (주)` | 이번 주 엔트리를 Notion DB 에 1엔트리=1행 동기화 + Week Summary DB (옵션) |
...
### 4.3 트리거
- 메인 페이지 `🔄 UpNote 저장 (주)` 클릭 → 확인 다이얼로그 → `POST /api/actions/upnote-sync`
...
**트리거**: 메인 페이지의 `🔄 Notion 동기화 (주)` 가 엔트리 + Week Summary 둘 다 같이 실행.
