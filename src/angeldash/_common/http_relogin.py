...
class AutoReloginHttp:
    ...
    async def _call_with_depth(
        ...
        # idle 기반 사전 재로그인 — 첫 시도(depth=0) 이고 refresh 진행 중이 아닐 때만.
        # 마지막 활동 이후 idle_timeout 초과면 응답 받기 전에 새 cookie 확보.
        if (
            depth == 0
            and not self._refreshing
            and self._can_refresh()
            and (time.time() - self._last_active) > self._idle_timeout
        ):
            idle_secs = time.time() - self._last_active
            logger.info(
                "Idle %.0fs > timeout %ds — pre-emptive relogin (url=%s)",
                idle_secs, self._idle_timeout, url,
            )
