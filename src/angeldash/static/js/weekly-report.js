...
// 자동 생성되는 휴가 행. 사용자가 직접 옮기거나 영구 삭제할 수 없게 ▲▼/<i data-lucide="trash-2"></i> 숨김.
...
let currentRows = [];
// 설정 캐시 — 이메일/UpNote/Notion 액션 시 사용
const cachedSettings = {
  weeklyNotebookId: '',
  upnoteWeeklyEnabled: false,
  notionWeeklyEnabled: false,
  notionWeeklyDbId: '',
  emailTo: '',
  emailCc: '',
...
async function loadCachedSettings() {
  try {
    ...
    cachedSettings.upnoteWeeklyEnabled =
      (s['upnote.enabled'] || 'false').toLowerCase() === 'true';
    cachedSettings.notionWeeklyEnabled =
      (s['notion.weekly_report_enabled'] || 'false').toLowerCase() === 'true';
    cachedSettings.notionWeeklyDbId =
      (s['notion.weekly_report_db_id'] || '').trim();
...
function renderRow(idx, normalMaxIdx) {
  ...
  // 액션 td: 휴가 행은 '자동' 라벨, 일반 행은 ▲ ▼ <i data-lucide="trash-2"></i>
...
document.getElementById('btn-preview-email').addEventListener('click', () => {
  ...
  openPreviewModal({
    title: '<i data-lucide="mail"></i> 이메일 미리보기',
    htmlForPreview: html,
    htmlForCopy: html,
    plainForCopy,
  });
...
// ─── UpNote / Notion 저장 + 사용 토글에 따른 버튼 표시 ───────

function applyWeeklySyncToggles() {
  const up = document.getElementById('btn-upnote-weekly');
  const no = document.getElementById('btn-notion-weekly');
  if (up) up.style.display = cachedSettings.upnoteWeeklyEnabled ? '' : 'none';
  if (no) no.style.display = cachedSettings.notionWeeklyEnabled ? '' : 'none';
}
...
document.getElementById('btn-notion-weekly').addEventListener('click', async () => {
  if (currentRows.length === 0) {
    toast('보고서가 비어있습니다');
    return;
  }
  if (!cachedSettings.notionWeeklyDbId) {
    toast('Notion 주간보고 DB ID 를 설정 페이지에서 먼저 등록하세요', 'fail');
    return;
  }
  try {
    const r = await apiPost('/api/actions/weekly-report-notion',
      { week_iso: currentWeek });
    toast(`Notion ${r.action === 'created' ? '신규 생성' : '갱신'} 완료 (${r.week_iso})`);
  } catch (e) {
    toast(`실패: ${e.message}`, 'fail');
  }
});
