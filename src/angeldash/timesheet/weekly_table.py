...
def _format_vacation_line(
    ...
    예 (반차/연차 모두 동일 규칙):
        종일 단일: '   . 홍길동 부장(04/21, 화)'
        종일 범위: '   . 홍길동 부장(02/02~06, 월~금)'
        반차 단일: '   . 홍길동 부장(04/21, 화, 오후)'
...
def render_table_cells(rows: list[dict]) -> tuple[list[str], list[list[str]]]:
    """Notion native 표용 — (headers, rows) 형식의 plain text 셀 배열을 반환.

    셀 안 줄바꿈은 `\\n` 로 유지 (Notion 이 그대로 렌더). markdown 강조나
    `<br>` / `&nbsp;` 같은 HTML 인코딩 없이 본문 그대로.
    """
    headers = list(_COL_HEADERS)
